...
We need a way to select actions from our policy. The solution depends on whether the actions for the environment are continuous or discrete.
For this tutorial, we assume our action will always be one from a set of discrete actions.

A common way to handle this is to interpret the outputs of the actor model as [logits](https://en.wikipedia.org/wiki/Logit) for each possible action.
Luckily, NumPy (and JAX) has a built-in function for picking a random action from a set of logits.
...
If n is 1, meaning it's the following observation, then this difference should be an estimate of the instantaneous reward at S.
...
The temporal difference error can improve the critic predictions by moving in the direction of the gradient at S, scaled by the TD error.
...
This also provides a way for our later state predictions to influence our earlier state predictions,
...
A consequence is that the value function doesn't converge to the actual reward approximation, but a point close to it called TD fixed point.
