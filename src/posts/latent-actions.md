...
title: LLM Latents as Continuous Actions
description: Experiement Proposal for LLM fine tuning
date: '2026-07-12'
published: true
---
...
LLM RL fine tuning typically trains token distributions using policy gradient losses. These token vocabularies are huge, ~150k-way discrete action space and the optimizer knows nothing about the relationship between these tokens. This is a hard exploration problem! Also we often don't care about the exact token selection per say, this is the wrong level of abstraction to think on. I'll argue it's possible to train a LLM at a higher level of abstraction, at a arbitrary layer of the network in latent space. Here I have a (as of know untested) idea for how to train a LLM with RL.
...
An interesting consequence is that there are now two sources of stochasticity:
    ...
    A latent-space sampler, which is part of the policy and provides exploration over continuous latent actions.
    The existing token sampler, which remains part of the environment dynamics rather than the policy itself.
...
This framing is related to prior work on action embeddings for discrete action spaces. Existing methods often map continuous actions to the nearest discrete embedding (“snapping”) (https://arxiv.org/abs/1512.07679). However, this creates hard boundaries: the critic only trains on the values of discrete actions and learns nothing about the space between them.
...
LLMs also conviently provide action embeddings in as their token embeddings, unlike most RL work (https://arxiv.org/pdf/1909.02291) it does not need to be learned.

I plan to explore this idea with my existing valm frame work: https://github.com/gabe00122/valm
